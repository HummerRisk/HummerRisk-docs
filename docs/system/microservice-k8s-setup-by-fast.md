...
!!! info "4. 使用外部的 MySQL 安装 hummerrisk"
    ...
    helm install hummerrisk hummerrisk/hummerrisk -n hummer --create-namespace  \
    --set hummerrisk.serviceType="NodePort" \
    --set global.storageClass="nfs"  \                # 你的存储类名称
    ...
    --set redis.enabled=false   \                     # 关闭创建 Redis 实例
    --set externalRedis.enabled=true  \               # 开启使用外部 Redis 数据库
    --set externalRedis.host="172.21.0.7"   \         
    --set externalRedis.port="6379"   \
    --set externalRedis.password="<your password>"   \
...
!!! info "通过 --set 配置参数"
    ...
    内部 Redis 数据库部分:
    --set redis.enabled=true                # 开启后会在 k8s 集群启动一个 Redis
    --set redis.password="password"         # 设置 Redis 密码
    外部 Redis 数据库部分:
    --set externalRedis.enabled=true        # 开启后直接连接外部 Redis
    --set externalRedis.host="192.168.10.101"     # 设置数据库地址，IP 或者域名
    --set externalRedis.port=6379           # 设置 Redis 端口，默认 6379
    --set externalRedis.password="<your password>"     # 设置数据库密码
    持久化存储配置:
