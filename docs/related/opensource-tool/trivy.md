![c7n](./img/../../img/../../img/related/trivycover.png){ width="80%" }   
### Trivy
...
!!! abstract "Trivy 类型"
    ...
    3. Trivy 可以在两种不同的模式下运行：
        * standalone
        * server-client
    4. 它旨在用于 CI 流程，在执行容器创建或部署应用程序之前，您可以轻松扫描本地容器镜像和其他内容。
...
!!! abstract "Trivy 特征"
    ...
    * 简易安装
      * apt-get install,yum install，brew install 均可安装 
      * 没有前置条件，如安装DB的，依赖库等
    * 高精确度
...
!!! tip "跳过漏洞数据库的更新，Trivy开始运行时每 12 小时下载一次漏洞数据库。这通常很快，因为数据库的大小只有 10~30MB。但是，如果您甚至想跳过它，请使用该--skip-db-update选项。"
    ```bash
