...
- feat（K8s 检测）：新增 K8s 检测规则组和规则，执行 K8s 合规检测，将按照规则组进行检测。

!!! summary "🚀 性能优化 Optimization"
- refactor（APIs）：优化 HummerRisk Restful APIs。
- refactor（资源态势）：K8s 资源态势，添加 xx 类型资源。
- refactor（镜像管理）：优化镜像管理列表数据展示。
- refactor（镜像仓库）：优化镜像仓库，同步的镜像数据直接添加到镜像管理列表里。
- refactor（云资源概览）：优化云资源概览页面，云账号统计数据的展示。
