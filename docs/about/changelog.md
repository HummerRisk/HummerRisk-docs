...
- feat（对象存储）：新增对象存储xx云类型，根据云账号，同步存储桶信息，并可以上传下载存储对象，风险检测存储桶。
- feat（操作审计）：新增操作审计xx云类型，根据xx云账号，同步操作审计数据，进而进行事件分析、聚合查询、源 IP 分析。
...
- fix (echarts）：修复echarts最新图表数据，更改字段大小写的问题。
