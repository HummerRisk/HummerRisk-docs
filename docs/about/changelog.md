...
!!! info "🌱 新功能 Features"
- feat（检测规则）：新增阿里云检测规则。
- feat（检测规则）：新增 AWS 检测规则。

!!! summary "🚀 性能优化 Optimization"
- refactor（消息管理）：优化云检测消息管理。
- refactor（漏洞库）：优化云原生漏洞库。
...
!!! info "🌱 新功能 Features"
- feat（报告中心）：定制整合报告功能，生成并下载为 PDF 格式。原下载报告菜单改名为云资源合规分析。
- feat（K8s 检测）：新增 Kubernetes、Rancher、KubeSphere 检测规则组以及规则。
- feat（账号管理）：新增云账号管理页面关联菜单功能，新增资源同步日志。

!!! summary "🚀 性能优化 Optimization"
- refactor（检测结果）：优化资源检测结果列表页面。
- refactor（规则列表）：优化规则列表页面，通过标签过滤数据功能。
- refactor（检测结果）：优化日志级别，修改日志执行检测的 log 输出。
- refactor（结果详情）：优化检测结果详情安全评分的计算与展示。
- refactor（规则列表）：优化规则列表（云资源检测规则、K8s 资源检测规则、主机检测规则）页面样式，调整排版。
- refactor（路由）：优化整体路由和跳转链接，由驼峰式命名修改为短横线命名，可读性更强、兼容性更好、符合标准化。
- refactor（URL）：优化 Controller 的 URL 地址，提高 URL 的可读性，使 URL 路径更直观，易于理解和记忆。

!!! success "🐛 Bug修复 Bug Fixes"
- fix（镜像检测）：修复镜像检测页面 log 错误。
- fix（检测详情）：修复云资源检测详情安全评分不正确的问题。
- fix（关于）：修复关于页面信息。
